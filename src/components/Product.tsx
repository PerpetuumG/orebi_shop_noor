...
const Product = ({ product, bg }: Props) => {
  return (
    <div
      className={
        'w-full relative group border-[1px] border-black hover:shadow-lg duration-200 shadow-gray-500 rounded-md overflow-hidden group'
      }
    >
      <div className={'w-full h-80 flex items-center justify-center bg-white overflow-hidden'}>
        <div className={`relative ${bg}`}>
          ...
          <div
            className={
              'abosute bottom-0 flex items-center gap-5 justify-center translate-y-[110%] group-hover:-translate-y-2 transition-transform duration-300'
            }
          >
            <button
              className={
          ...
          {product?.isnew && (
            <div className={'absolute top-2 right-2 z-50'}>
              <p
                className={
                  'bg-primeColor px-4 py-1 text-white flex justify-center items-center text-sm font-semibold hover:bg-black duration-300 cursor-pointer rounded-md'
                }
              >
                New
              </p>
      ...
      <div className={'max-w-80 py-6 flex flex-col gap-1 px-4'}>
        ...
        <div className={'flex items-center justify-between'}>
          <p className={'text-[#767676] text-sm'}>
            a product by <span className={'font-semibold text-primeColor'}>{product?.brand}</span>
          </p>
